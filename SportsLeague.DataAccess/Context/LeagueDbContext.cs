...

    {
...

        {
            ...
            modelBuilder.Entity<MatchLineup>(entity =>
            {
                ...
                // Relación con Match (1:N)
                entity.HasOne(ml => ml.Match)
                    .WithMany(m => m.MatchLineups)  // ⚠️ Vamos a agregar esta propiedad en Match después
                    .HasForeignKey(ml => ml.MatchId)
                    .OnDelete(DeleteBehavior.Cascade);

                // Relación con Player (1:N)
                entity.HasOne(ml => ml.Player)
                    .WithMany(p => p.MatchLineups)  // ⚠️ Vamos a agregar esta propiedad en Player después
